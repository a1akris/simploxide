...
impl Transmitter {
    /// Returns an error if web socket is disconnected
...
/// This redirects the entire stream of requests to the web socket. Requests are flushed only when
/// `requests.poll()` returns pending or when the internal websocket buffer(which is 64MB by
/// default) is full so, effectively, requests are buffered in the stream and being transmitted in
/// batches if there is a huge supply of them.
